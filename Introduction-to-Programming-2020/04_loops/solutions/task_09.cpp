...
void printSymbolOfChosenCard(short positionEntered, short SELECTED_COLOR);

void printColorOfChosenCard(short SELECTED_COLOR);
...
void printSymbolOfChosenCard(short positionEntered, short SELECTED_COLOR) {
    ...
    for (int position = FIRST_VALID_POSITION; position < TOTAL_SYMBOLS_COUNT; ++position) {
        ...
        switch (currentPosition) {
            ...
            default:
                isSymbolFound = true;
                const int INTEGER_SYMBOL = currentPosition + 1; // Integer from the set [2; 10]
